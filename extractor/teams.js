...
function setupForTeam() {
  ...
  console.log("setupForTeam: tables found =", tables.length, tables);

  if (!tables[0]) {
    console.warn("setupForTeam: tables[0] not found");
    return;
  }

  let tbody = tables[0].querySelector("tbody");
  if (!tbody) {
    console.warn("setupForTeam: tbody not found in tables[0]");
    return;
  }

  let tbodyrows = tbody.children;
  console.log("setupForTeam: tbodyrows length =", tbodyrows.length, tbodyrows);

  let teamnationelem = document.getElementsByClassName("country-name")[0];
  if (!teamnationelem) {
    console.warn("setupForTeam: .country-name not found");
    return;
  }

  let teamnation = teamnationelem.textContent;
  console.log("setupForTeam: teamnation =", teamnation);

  let currentdefaultpos = "GK";
  console.log("setupForTeam: initial currentdefaultpos =", currentdefaultpos);

  for (let i = 0; i < tbodyrows.length; i++) {
    let row = tbodyrows[i];
    console.log(
      "setupForTeam: row index",
      i,
      "class=",
      row.className,
      "children=",
    ...
    if (isHeaderRow(row)) {
      ...
      if (row.classList && row.classList.contains("hs-repeated-header")) {
        console.log("  -> hs-repeated-header, skipping");
        continue;
      }
      ...
      let headerText =
        row.children[0] && row.children[0].textContent
          ...
          : "";
      console.log("  -> position header text =", headerText);

      currentdefaultpos = processdefaultpos(headerText);
      console.log("  -> updated currentdefaultpos =", currentdefaultpos);

      continue;
    }

    console.log(
      "  -> not a header row, using currentdefaultpos =",
      currentdefaultpos
    );

    if (currentdefaultpos == "stop") {
      console.log("  -> currentdefaultpos is 'stop', breaking");
      break;
    }

    console.log("  -> processing player row");
    processPlayerRow(row, currentdefaultpos, teamnation);
  }

  console.log("setupForTeam: finished loop, calling createPlayerInfoPanel");
...
function processPlayerRow(row, currentdefaultpos, teamnation) {
  //console.log(row.children[2].children[0].href);
  
  console.log('Is this player row?', row.children[2]);

  if (row.children[2].tagName === 'TD') {//checks if this is a player row at all

    console.log('what is playername', row.children[2].children[0].textContent);
...
function populatePlayerData() {
  const tbody = document
    .getElementById("playerDataTable")
  ...
  teamplayers.forEach((player) => {
    ...
    row.appendChild(
      createSelectCell(player.nationality || "Côte d’Ivoire", nationOptions)
    );
    ...
    row.appendChild(
      createSelectCell(
        player.transferval || "{Ignore}",
        getTransferValueOptions()
      )
    );
    row.appendChild(createNumberCell(player.ovr || 62, 1, 99));
    row.appendChild(
      createSelectCell(player.position1 || "CM", getPositionOptions())
    );
    row.appendChild(
      createSelectCell(player.position2 || "", getPositionOptions(true))
    );
    row.appendChild(
      createSelectCell(player.position3 || "", getPositionOptions(true))
    );
    row.appendChild(
      createSelectCell(player.position4 || "", getPositionOptions(true))
    );
    row.appendChild(createSelectCell(player.foot || "Right", getFootOptions()));
    row.appendChild(
      createSelectCell(player.weakfoot || "Average", getWeakFootOptions())
    );
...
function findMatchingNation(inputNation) {
  ...
  const matchedNation = nationList.find(
    (nationObj) =>
