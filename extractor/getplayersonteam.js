async function fetchplayers() {
  ...
  for (const player of teamplayers) {
    ...
    (document.getElementById("playerInfoPanel").innerHTML +=
      "<br>" + "Processing: " + player.given + " " + player.sur),
      player.url + "...";
    ...
    await new Promise((resolve, reject) => {
      iframe.onload = async function () {
        try {
          const result = await getplayerdata(
            ...
            player.url
          ); // Pass the iframe's document to the function
          console.log("result");
          results.push(result);
          resolve(); // Resolve the promise to move on to the next player
        } catch (error) {
          console.error(
            "Error extracting data for player:",
            player.given + " " + player.sur,
            error
          );
      ...
      iframe.onerror = function (error) {
        console.error(
          "Error loading iframe for player:",
          player.given + " " + player.sur,
          error
        );
        resolve(); // Resolve on error to continue processing
...
async function getplayerdata(iframeDocument, playerurl) {
  ...
  const playerName = playerNameElement
    ? playerNameElement.innerText.trim()
    : "unknown_player";

  let dataObj = { playerName, pageurl: playerurl };

  // NEW LAYOUT: <dl><dt>Key</dt><dd>Value</dd>...</dl>
  const factsdl =
    iframeDocument.querySelector(".hs-profile.hs-profile--person dl") ||
    iframeDocument.querySelector(".hs-profile--person dl") ||
    iframeDocument.querySelector(".hs-profile dl");
  ...
  if (factsdl) {
    ...
    dts.forEach(function (dt) {
      ...
      if (!dd) {
        console.error("dt without dd", dt);
        return;
      }
      ...
      const span = dd.querySelector('span[itemprop="nationality"]');
      if (span) {
  ...
  // FALLBACK: old table layout (kept in case some pages still use it)
  const table = iframeDocument.querySelector(".standard_tabelle.yellow");
  if (!table) {
    console.log("No facts <dl> or table found.");
    return;
  }
...
function updatePlayer(result) {
  if (result) {
    let thisplayer = teamplayers.find(
      (player) => player.url === result.pageurl
    );
...
function processPositions(player, value) {
  // value might be like "Left Back, Left Midfielder" OR use <br>
  var normalized = (value || "").replace(/<br\s*\/?>/gi, ",");
  var positions = normalized
    .split(",")
    .map(function (pos) { return pos.trim(); })
    .filter(function (pos) { return pos.length > 0; });

  console.log("processPositions raw:", value, "parsed:", positions);
